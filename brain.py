...
class Brain:
    def __init__(self, inputs, clone=False):
        self.connections = []
        self.nodes = []
        self.inputs = inputs # There are 3 inputs in this example
        self.net = [] # Nodes that appear in the neural network ordered by layer (inputs first, output last)
        self.layers = 2
    ...
    def connect_nodes(self):
        for i in range(0, len(self.nodes)):
            self.nodes[i].connections = [] # Make sure there are no connections from prior generations
    ...
    def generate_net(self):
        self.connect_nodes()
        self.net = [] # Clear the net from prior generation
